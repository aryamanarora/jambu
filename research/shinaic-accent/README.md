...
The user cancelled the blogpost. Its source, blog listing, and changelog announcement
have been removed, and [publication PR #1](https://github.com/aryamanarora/jambu/pull/1)
is closed without merging. The investigation remains here as analysis code, notes,
and evidence. The former web downloads now live in `evidence/` in this directory;
the packaging and verification scripts use that local path. Earlier delivery and
publication notes below describe the historical work, not a pending publication.

The six analysis scripts, in run order, are `inventory.py`, `audit_palula.py`,
`compare_shina.py`, `annotate_families.py`, `annotate_palula_nominals.py`, and
`survey_comparison.py`. They use Python 3's standard library and read the sibling
`data` repository by default; set `JAMBU_DATA` to use another checkout.
`package_evidence.py` packages the outputs locally, and `verify_package.py` replays
the preserved archive in a temporary directory and compares its outputs.
