...
- [ ] Complete applicable ingestion-checklist audits before shipping data changes.
  Verify source spelling, locators, source-specific profile routing, bibliography,
  language/dialect parents and clades, and explicit graph claims. Apply the user's
  editorial decisions without silently overriding them with a default classification.
- [ ] Back up/record pre-build IDs, source keys, aliases and graph counts. Run focused
  tests first and the current full data pipeline in order (`make all` when configured).
