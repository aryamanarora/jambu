// @ts-nocheck
// Vendored fzstd 0.1.1. Offset fields can span five bytes; 32-bit reads lost high bits.
// See LICENSE.fzstd and the browser-decoder round-trip test.
import { readOffsetBits } from '../zstdBits.js';
    ...
    if (btype == 2) {
        ...
        if (ns) {
            ...
            for (++ns; --ns;) {
                ...
                cbt = (spos -= ofc) >> 3;
                var ofp = 1 << ofc;
                var off = ofp + readOffsetBits(dat, cbt, spos & 7, ofc);
                cbt = (spos -= mlb[mlc]) >> 3;
