<script lang="ts">
	...
	const selected = $derived(
		references.find((reference) => reference.id === value || reference.short === value) ?? null
	);
		...
		<div class="panel" use:floatingPanel={triggerEl}>
			<div class="search-row">
				...
				<input
					...
					autofocus
				/>
				{#if value}<button class="clear" onclick={() => pick('')}>clear</button>{/if}
			</div>
			<div class="list">
				<button class="option any" class:on={!value} onclick={() => pick('')}>Any source</button>
				{#if !loaded}<div class="hint">loading references…</div>{/if}
				{#each filtered as reference (reference.id)}
					<button
						class="option"
						class:on={value === reference.id || value === reference.short}
						onclick={() => pick(reference.id)}
					>
...
<style>
	...
	@media (max-width: 640px) {
		.trigger, .option { min-height: 42px; }
